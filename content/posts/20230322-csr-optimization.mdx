...
Tree Shaking 是一種將沒有使用的 JavaScript 代碼從 bundle 文件中刪除的技術，可以減少 bundle 文件的大小，從而提高 CSR 效能。在 React 應用中，可以使用 babel-plugin-transform-react-remove-prop-types 插件來實現 Tree Shaking。例如：

```jsx .balelrc showLineNumbers
// .babelrc
{
  "plugins": [
...
在這個例子中，使用 babel-plugin-transform-react-remove-prop-types 插件將 React 組件中沒有使用的 prop-types 從 bundle 文件中刪除，從而減少 bundle 文件的大小，提高 CSR 效能。
