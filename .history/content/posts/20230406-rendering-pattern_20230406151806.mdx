...
圖片來源: [Rendering on the Web](https://google.com)
server 回傳只包含一個`<div>`的 html，根據 id 下載對應的 js 檔案，解析並執行 react code，最後才會 render 畫面。

在 js 載入後，才會將資料塞入到`<div>`中

2. 重度客戶端應用：對於需要豐富客戶端交互的應用，CSR 可以通過 JavaScript 代碼的編寫和執行來實現，從而實現更好的用戶體驗。

需要注意的是，CSR 的主要缺點是對 SEO 不友好，因此如果您的網站需要良好的 SEO 結果，則需要考慮使用其他渲染方式，例如 SSR 或 SSG。
