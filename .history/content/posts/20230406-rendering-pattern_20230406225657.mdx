...
- SSR 可以提高首次加載速度，因為瀏覽器只需要下載 HTML，CSS 和 JavaScript 靜態文件。
- SSR 更有利於 SEO，因為搜尋引擎可以更容易地獲取 HTML 內容。
- 在低端設備上的性能更好，因為需要更少的客戶端運算。

缺點：

- SSR 的服務器負載較高，需要更多的計算資源。
- SSR 的數據載入速度可能比 CSR 慢，因為需要在服務器上進行渲染，而服務器可能會有更多的延遲時間。
- 在開發中，必須小心處理服務器端和客戶端的代碼，以確保兩者間的一致性。
