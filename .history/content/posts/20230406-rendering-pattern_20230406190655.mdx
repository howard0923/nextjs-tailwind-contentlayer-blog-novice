...
server 回傳只包含一個`<div>`的 html ， FCP(First Contentful Paint 渲染出的第一個內容，由於 CSR 頁面都只有一個根結點，所以常常會)，根據 id 下載對應的 js 檔案，解析並執行 react code，解析完畢才會 render 畫面，最後才達到 TTI(Time to Interactive 使用者可以開始跟頁面互動)。
