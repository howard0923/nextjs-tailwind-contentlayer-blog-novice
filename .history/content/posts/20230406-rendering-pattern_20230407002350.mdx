...
在 SSG 中，網站的內容可以事先被編譯成靜態文件，然後放到 CDN（Content Delivery Network，內容分發網路，將網路流量分佈到各個服務器上，以減少單一服務器上的負載，提高網頁載入速度和用戶體驗。當用戶訪問網站時，CDN 會根據用戶的地理位置自動選擇最近的節點，從而提高頁面載入速度，降低網路延遲和丟包率。）或靜態網頁伺服器上，讓用戶直接訪問靜態頁面，不需要透過伺服器動態生成。
